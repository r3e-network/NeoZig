...
test "Governance method script generation" {
    ...
    for (governance_methods) |method| {
        ...
        _ = try script_builder.contractCall(neo_script_hash, method, &[_]ContractParameter{});
...
test "NEO token governance operations" {
    ...
    _ = try get_gas_builder.contractCall(neo_script_hash, GET_GAS_PER_BLOCK, &[_]ContractParameter{});
    ...
    _ = try get_price_builder.contractCall(neo_script_hash, GET_REGISTER_PRICE, &[_]ContractParameter{});
...
test "NEO token method name validation" {
    ...
    const valid_methods = [_][]const u8{
        ...
        VOTE,               REGISTER_CANDIDATE, UNREGISTER_CANDIDATE, GET_GAS_PER_BLOCK, SET_GAS_PER_BLOCK,
        GET_REGISTER_PRICE, SET_REGISTER_PRICE, GET_ACCOUNT_STATE,
    };
    ...
    for (invalid_methods) |method| {
        ...
        if (std.mem.eql(u8, method, "")) {
            ...
            try testing.expectError(@import("../../src/core/errors.zig").NeoError.IllegalArgument, neo_token.validateInvocation(method, &empty_params));
        } else {
