...
pub const NeoZig = struct {
    ...
    pub fn getNeoSwiftRx(self: *Self) *JsonRpc2_0Rx {
        if (self.neo_swift_rx == null) {
            self.neo_swift_rx = JsonRpc2_0Rx.init(self, self.config.polling_interval);
        }
    ...
    pub fn format(self: Self, allocator: std.mem.Allocator) ![]u8 {
        ...
        return try std.fmt.allocPrint(
            allocator,
            "NeoZig(network: {s}, block_time: {}ms, polling: {}ms, nns_resolver: available)",
            .{ 
                network_name, 
                self.getBlockInterval(), 
                self.getPollingInterval() 
            }
        );
    }
...
test "NeoZig creation and configuration" {
    ...
    const service = NeoSwiftService.init(allocator); // Mock service
...
test "NeoZig factory methods" {
    ...
    const service = NeoSwiftService.init(allocator);
    defer service.deinit();
    
    // Test MainNet client
    var mainnet_client = NeoZig.Factory.createMainNet(service, allocator);
    ...
    // Test TestNet client
    var testnet_client = NeoZig.Factory.createTestNet(service, allocator);
    ...
    var dev_client = NeoZig.Factory.createDev(service, allocator);
...
test "NeoZig reactive client access" {
    ...
    try testing.expect(rx_client.getPollingInterval() > 0);
