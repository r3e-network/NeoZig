...
pub const NNSName = struct {
    ...
    pub fn getSubdomains(self: Self, allocator: std.mem.Allocator) ![][]u8 {
        ...
        while (fragment_iterator.next()) |fragment| {
            const owned_fragment = try allocator.dupe(u8, fragment);
            errdefer allocator.free(owned_fragment);
            try subdomains.append(owned_fragment);
        }
...
test "NNSName error handling" {
    ...
    try testing.expectError(errors.NeoError.IllegalArgument, NNSName.init("too.many.sub.domain.levels.here.more.levels.neo", allocator));

    try testing.expectError(errors.NeoError.IllegalArgument, NNSName.init("-invalid.neo", allocator));
}
