    ...
    pub fn getErrorDescription(self: Self, allocator: std.mem.Allocator) ![]u8 {
        return switch (self) {
            ...
            .UnexpectedReturnType => |data| {
                if (data.expected_types) |expected| {
                    ...
                    return try std.fmt.allocPrint(
                        ...
                        .{ data.actual_type, expected_str.items }
                    );
                } else {
            ...
            .UnresolvableDomainName => |name| {
                return try std.fmt.allocPrint(
                    allocator, 
                    "The provided domain name '{s}' could not be resolved.", 
                    .{name}
                );
            },
        };
...
test "ContractErrorUtils validation functions" {
    ...
    try testing.expectError(
        ContractError.InvalidNeoName,
        ContractErrorUtils.validateContractName("")
    );
    
    try testing.expectError(
        ContractError.InvalidNeoName,
        ContractErrorUtils.validateContractName("x" ** 300)
    );
    ...
    try testing.expectError(
        ContractError.InvalidNeoNameServiceRoot,
        ContractErrorUtils.validateNNSRoot("invalid")
    );
    ...
    try testing.expectError(
        ContractError.UnexpectedReturnType,
        ContractErrorUtils.validateStackItemType("Boolean", &valid_types)
    );
    ...
    try testing.expectError(
        ContractError.UnresolvableDomainName,
        ContractErrorUtils.checkDomainResolvable("")
    );
    
    try testing.expectError(
        ContractError.UnresolvableDomainName,
        ContractErrorUtils.checkDomainResolvable("nodomain")
    );
}