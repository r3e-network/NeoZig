...
pub const ExpressInfo = struct {
    ...
    pub fn format(self: ExpressInfo, allocator: std.mem.Allocator) ![]u8 {
        return try std.fmt.allocPrint(allocator, "Express {s} ({s}) - Running: {}, Blocks: {}", .{ self.version, self.network_type.toString(), self.is_running, self.block_count });
    }
};
...
pub const ExpressUtils = struct {
    ...
    pub fn createExpressUrl(host: ?[]const u8, port: ?u16, allocator: std.mem.Allocator) ![]u8 {
        ...
        return try std.fmt.allocPrint(allocator, "http://{s}:{d}", .{ actual_host, actual_port });
    }
    ...
    pub fn createDefaultCheckpointName(allocator: std.mem.Allocator) ![]u8 {
        ...
        return try std.fmt.allocPrint(allocator, "checkpoint_{d}.acc", .{timestamp});
    }
};
...
test "NeoExpress checkpoint operations" {
    ...
    try testing.expectError(errors.ValidationError.RequiredParameterMissing, ExpressUtils.validateCheckpointFilename(""));

    try testing.expectError(errors.ValidationError.ParameterOutOfRange, ExpressUtils.validateCheckpointFilename("x" ** 300));

    try testing.expectError(errors.ValidationError.InvalidParameter, ExpressUtils.validateCheckpointFilename("invalid@checkpoint.acc"));
