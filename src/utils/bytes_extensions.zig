...
test "BytesUtils padding operations" {
    ...
    try testing.expectError(errors.NeoError.IllegalArgument, BytesUtils.toPadded(&short_bytes, 1, false, allocator));
}
...
test "BytesUtils script hash to address" {
    ...
    try testing.expectError(errors.NeoError.IllegalArgument, BytesUtils.scripthashToAddress(&invalid_script_hash, allocator));
}
