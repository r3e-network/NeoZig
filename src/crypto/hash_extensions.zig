    ...
    pub fn benchmarkHashOperations(data: []const u8, iterations: u32) HashBenchmark {
        if (iterations == 0) {
            return HashBenchmark{
                .sha256_ns_per_op = 0,
                .ripemd160_ns_per_op = 0,
                .double_sha256_ns_per_op = 0,
                .iterations = 0,
            };
        }
        ...
        const divisor: u64 = @intCast(iterations);
        ...
        return HashBenchmark{
            .sha256_ns_per_op = sha256_time / divisor,
            .ripemd160_ns_per_op = ripemd160_time / divisor,
            .double_sha256_ns_per_op = double_sha256_time / divisor,
            .iterations = iterations,
        };
    }
};
...
test "Hash validation operations" {
    ...
    try testing.expectError(errors.ValidationError.InvalidHash, StringHashUtils.validateHashString("invalid", 20));

    try testing.expectError(errors.ValidationError.InvalidHash, StringHashUtils.validateHashString("1234567890abcdef", 20) // Too short
    );

    try testing.expectError(errors.ValidationError.InvalidHash, StringHashUtils.validateHashString("1234567890abcdef1234567890abcdef123456789abcdef", 20) // Too long
    );
}
