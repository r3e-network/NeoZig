...
pub const Ripemd160 = struct {
    ...
    pub const FinalError = error{OutputTooSmall};

    pub fn final(self: *Self, out: []u8) void {
        self.finalChecked(out) catch @panic("Ripemd160.final: output buffer too small");
    }

    pub fn finalChecked(self: *Self, out: []u8) FinalError!void {
        if (out.len < 20) return FinalError.OutputTooSmall;
...
test "Ripemd160.finalChecked rejects short output buffers" {
    const testing = std.testing;
    ...
    try testing.expectError(Ripemd160.FinalError.OutputTooSmall, hasher.finalChecked(out[0..]));
}
