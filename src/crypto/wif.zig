...
pub fn encode(private_key: PrivateKey, compressed: bool, network: NetworkType, allocator: std.mem.Allocator) ![]u8 {
    const payload_size: usize = 1 + constants.PRIVATE_KEY_SIZE + (if (compressed) @as(usize, 1) else @as(usize, 0)) + 4;
    ...
    @memcpy(payload[pos .. pos + constants.PRIVATE_KEY_SIZE], private_key.toSlice());
    pos += constants.PRIVATE_KEY_SIZE;
