...
pub const ECKeyPair = struct {
    ...
    pub fn signAndGetECDSASignature(self: Self, message_hash: []const u8) ECDSASignature {
        // Convert message hash to Hash256
        var hash_bytes: [32]u8 = undefined;
        @memcpy(&hash_bytes, message_hash[0..@min(32, message_hash.len)]);
        
        const hash256 = Hash256.initWithBytes(&hash_bytes) catch Hash256.ZERO;
    ...
    pub fn verifySignature(self: Self, message_hash: []const u8, signature: ECDSASignature) !bool {
        ...
        const hash256 = try Hash256.initWithBytes(&hash_bytes);
