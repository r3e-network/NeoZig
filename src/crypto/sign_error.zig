...
test "SignErrorUtils utility functions" {
    ...
    try testing.expectError(
        ...
        SignErrorUtils.validateSignatureComponents(0, 1) // Invalid R
    );
    
    try testing.expectError(
        SignError.RecoverFailed,
        SignErrorUtils.validateSignatureComponents(1, 0) // Invalid S
    );
    
    try testing.expectError(
        SignError.RecoverFailed,
        SignErrorUtils.validateSignatureComponents(secp256r1.Secp256r1.N, 1) // R too large
    );
    ...
    try testing.expectError(
        SignError.HeaderOutOfRange,
        SignErrorUtils.validateRecoveryParameters(5, &valid_hash) // Invalid recovery ID
    );
    
    const invalid_hash = [_]u8{0xCD} ** 16; // Wrong length
    try testing.expectError(
        SignError.RecoverFailed,
        SignErrorUtils.validateRecoveryParameters(1, &invalid_hash)
    );
}
...
test "SignError conversion and handling" {
    ...
    try testing.expectError(
        errors.CryptoError.SignatureVerificationFailed,
        SignErrorUtils.handleSignatureError("test_sign", error.RecoverFailed, allocator)
    );
}
