...
//!
//! Neo N3 
//! 160-bit hash type for Neo N3 script hashes and addresses.
...
pub const Hash160 = struct {
    /// Raw hash bytes stored in big-endian order
    ...
    /// Creates Hash160 from public key
    ...
    /// Creates Hash160 from multiple public keys
    ...
    /// Creates Hash160 from hexadecimal string
...
fn addressToScriptHash(address: []const u8, allocator: std.mem.Allocator) ![]u8 {
    ...
    // Extract script hash (skip version byte) and convert to big-endian.
...
/// Build multi-signature verification script
...
test "Hash160 known Neo address" {
    ...
    // Hash160 strings are stored/represented in big-endian order (matches NeoClient).
