...
test "RecordType conversion operations" {
    ...
    try testing.expectError(errors.NeoError.IllegalArgument, RecordType.throwingValueOf(99));
}
...
test "RecordType data validation" {
    ...
    try testing.expectError(errors.ValidationError.InvalidParameter, RecordType.A.validateRecordData("invalid.ipv4.address.here"));

    try testing.expectError(errors.ValidationError.InvalidParameter, RecordType.A.validateRecordData("192.168.1") // Missing octet
    );
    ...
    try testing.expectError(errors.ValidationError.InvalidParameter, RecordType.AAAA.validateRecordData("invalid::ipv6::address::format"));
    ...
    try testing.expectError(errors.ValidationError.ParameterOutOfRange, RecordType.TXT.validateRecordData("x" ** 2000) // Too long
    );
}
...
test "RecordTypeUtils detection and validation" {
    ...
    try testing.expectError(errors.ValidationError.InvalidParameter, RecordTypeUtils.validateRecordTypeAndData(.A, "not.an.ip.address"));
}
