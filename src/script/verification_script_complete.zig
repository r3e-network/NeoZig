...
test "CompleteVerificationScript error conditions" {
    ...
    try testing.expectError(errors.NeoError.IllegalArgument, CompleteVerificationScript.initFromPublicKeys(&single_key, 0, allocator));

    // Test threshold too high
    try testing.expectError(errors.NeoError.IllegalArgument, CompleteVerificationScript.initFromPublicKeys(&single_key, 2, allocator));
    ...
    try testing.expectError(errors.NeoError.IllegalArgument, CompleteVerificationScript.initFromPublicKeys(&many_keys, 1, allocator));
}
...
test "VerificationScriptUtils utility functions" {
    ...
    try testing.expectError(errors.NeoError.IllegalArgument, VerificationScriptUtils.validateMultiSigConfig(&public_keys, 0));

    try testing.expectError(errors.NeoError.IllegalArgument, VerificationScriptUtils.validateMultiSigConfig(&public_keys, 2));
