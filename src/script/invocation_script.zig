...
pub const ScriptAnalysis = struct {
    ...
    pub fn format(self: ScriptAnalysis, allocator: std.mem.Allocator) ![]u8 {
        return try std.fmt.allocPrint(
            allocator,
            "Script: {} bytes, {} opcodes, {} pushes, {} signatures",
            .{ self.total_bytes, self.opcode_count, self.push_operations, self.signature_count }
        );
    }
...
test "InvocationScriptUtils utility functions" {
    ...
    try testing.expectError(
        ...
        InvocationScriptUtils.createMultiSignatureScript(&multi_signatures, 5, allocator)
    );
