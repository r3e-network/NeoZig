...
pub const ScriptAnalysis = struct {
    ...
    pub fn format(self: ScriptAnalysis, allocator: std.mem.Allocator) ![]u8 {
        return try std.fmt.allocPrint(allocator, "Script Analysis: {} bytes, {} opcodes (Push: {}, Syscall: {}, Jump: {}, Arithmetic: {})", .{ self.total_bytes, self.opcodes.items.len, self.push_operations, self.syscall_operations, self.jump_operations, self.arithmetic_operations });
    }
};
...
test "ScriptReader validation" {
    ...
    try testing.expectError(errors.ValidationError.InvalidScript, ScriptReader.validateScriptStructure(&large_script, allocator));
